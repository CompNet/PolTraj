...
# plot n°1
plot.filename <- file.path(data.folder,"fig1.pdf")
...
# plot n°2
plot.filename <- file.path(data.folder,"fig2.pdf")
pdf(file=plot.filename,bg="white",compress=FALSE)
	par(mfrow=c(2,2))
...
# plot n°3: State distribution within each cluster
plot.filename <- file.path(data.folder,"fig3.pdf")
pdf(file=plot.filename,bg="white",compress=FALSE)
	seqdplot(traj.seq, group=cl4.fac, border=NA)
dev.off()

# plot n°4: Representative sequence of each cluster
plot.filename <- file.path(data.folder,"fig4.pdf")
pdf(file=plot.filename,bg="white",compress=FALSE)
...
# plot n°5: Association between the state sequences and the selected covariate
plot.filename <- file.path(data.folder,"fig5.pdf")
...
# plot n°6: Evolution of the within-group discrepancy
plot.filename <- file.path(data.folder,"fig6.pdf")
pdf(file=plot.filename,bg="white",compress=FALSE)
...
plot.filename <- file.path(data.folder,"fig7.png")
st <- seqtree(traj.seq ~ Sexe + `Groupe politique` + Profession, data=perso,
		R=5000, diss=traj.dist, pval = 0.05)
seqtreedisplay(st, filename=plot.filename, type="d", border=NA, gvpath='D:/Program Files (x86)/Graphviz2.38')

# plot n°8: Most frequent subsequences
plot.filename <- file.path(data.folder,"fig8.pdf")
...
plot.filename <- file.path(data.folder,"fig9.pdf")
